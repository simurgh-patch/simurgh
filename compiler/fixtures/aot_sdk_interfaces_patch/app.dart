...
class Rank implements Comparable<Rank> {
  final int value;
  Rank(this.value);
  int compareTo(Rank other) => other.value.compareTo(value);
}
...
Comparable<Rank> make() => AddedRank(5);
Iterator<int> iterator() => Numbers();
Exception failure() => Trouble();
String ordered() {
...
class AddedRank extends Rank {
  AddedRank(super.value);
  int compareTo(Rank other) => other.value - value;
}

class Numbers implements Iterator<int> {
  int index = 0;
  int get current => index * 10;
  bool moveNext() {
    index++;
    return index <= 3;
  }
...
class Trouble implements Exception {
  String toString() => 'new failure';
}
