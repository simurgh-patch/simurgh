T choose<T>(T first, T second) => first;
...
num bounded<T extends num>(T first, T second) => first;
String typed<T>(T value) => '$T:$value';
Future<T> later<T>(T first, T second) async {
  await Future<void>.value();
  return first;
}
T Function<T>(T, T) make() => <T>(T first, T second) => first;
T Function(T) capture<T>(T seed) => (T next) => seed;
class Selector {
  T pick<T>(T first, T second) => first;
}
