...
class Base<T> {
  final T value;
  final int extra;
  const Base(this.value, [this.extra = 20]);
  const Base.named({required this.value, this.extra = 30});
  const Base._hidden(this.value) : extra = 4;
  T read() => value;
}

mixin Label<T> on Base<T> {
  String label() => 'patched:${super.read()}:${extra}';
}
...
class Added<T> = Base<T> with Label<T>;
