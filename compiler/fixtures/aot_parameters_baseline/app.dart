int adjust(int value, {int scale = 2, required int bias}) => value * scale + bias;
int optional(int value, [int delta = 3]) => value + delta;
...
int Function(int, {int increment}) make(int start) {
  return (int value, {int increment = 2}) => start + value + increment;
}
class Base {
  int offset;
  Base({this.offset = 4});
...
class Child extends Base {
  ...
  int compute(int value, {int scale = 2, int bias = 1}) => super.compute(value, scale: scale, bias: bias) + 5;
}
