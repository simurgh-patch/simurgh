class Box<T> {
  ...
  String typeName() => 'patched:$T';
  String shadow<T>(T next) => 'patched:$T:$next';
  U map<U>(U Function(T) callback) => callback(value);
  Future<T> later(T next) async { await Future<void>.value(); return next; }
}
...
class Added<V> extends Box<V> {
  Added(V value) : super(value);
  V pick(V next) => next;
}
Box<int> make(int value) => Added<int>(value);
