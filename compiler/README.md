...
Pass the resulting directories as `--top-accessors-run-dir`, `--top-accessor-gc-run-dir`, `--top-accessor-metadata-run-dir`, and `--top-accessor-metadata-relink-run-dir` to `scripts/verify_aot_lab.py`, together with its required base and GC runs. The verifier compares both sides against independently compiled original-source AOT, checks retained AOT callers and GC behavior, and compares source, wrapper, and helper annotations in actual Kernel. This is host ARM64 experimental coverage, not general Flutter or mobile acceptance.
