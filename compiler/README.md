...
Explicit top-level getters and setters in the restricted source graph now get separate stable execution helpers behind a generated static property wrapper. The wrapper retains native assignment, compound update, increment, null-aware assignment and getter-returned function-call semantics for unchanged AOT callers. Accessor identity is bound to the owning library, including parts and private declarations; identical names in different libraries do not collide. Compound updates collect both resolved read and write elements, and distinct execution symbols without a shared property wrapper fail closed. Inferred getter returns keep the analyzer's resolved type, including `dynamic` for an untyped getter. Declaration and setter-parameter constant annotations are retained on both the property wrapper and replaceable helper, including supported VM pragmas. Changing accessor annotations versions the property wrapper and rebinds its AOT consumers. External top-level accessors remain rejected.

```sh
python3 scripts/aot_lab.py --baseline compiler/fixtures/aot_top_accessors_baseline/app.dart --candidate compiler/fixtures/aot_top_accessors_patch/app.dart
python3 scripts/aot_lab.py --baseline compiler/fixtures/aot_top_accessor_gc_baseline/app.dart --candidate compiler/fixtures/aot_top_accessor_gc_patch/app.dart
python3 scripts/aot_lab.py --baseline compiler/fixtures/aot_top_accessor_metadata_baseline/app.dart --candidate compiler/fixtures/aot_top_accessor_metadata_patch/app.dart
python3 scripts/aot_lab.py --baseline compiler/fixtures/aot_top_accessor_metadata_baseline/app.dart --candidate compiler/fixtures/aot_top_accessor_metadata_relink_patch/app.dart
python3 scripts/aot_lab.py --baseline compiler/fixtures/aot_package_accessors_baseline/app.dart --candidate compiler/fixtures/aot_package_accessors_patch/app.dart
python3 scripts/aot_lab.py --baseline compiler/fixtures/aot_top_accessor_signature_baseline/app.dart --candidate compiler/fixtures/aot_top_accessor_signature_patch/app.dart
```

Pass the resulting directories as `--top-accessors-run-dir`, `--top-accessor-gc-run-dir`, `--top-accessor-metadata-run-dir`, `--top-accessor-metadata-relink-run-dir`, `--top-accessor-package-run-dir`, and `--top-accessor-signature-run-dir` to `scripts/verify_aot_lab.py`, together with its required base and GC runs. The verifier compares both sides against independently compiled original-source AOT, checks retained AOT callers and GC behavior, and compares source, wrapper, and helper annotations in actual Kernel. The package fixture splits getter and setter across a package library and its part; its archived package source is compiled independently, while application and package callers remain AOT. A getter/setter type change versions the property wrapper and rebinds its callers; an unrelated public function signature change still requires a new baseline. This is host ARM64 experimental coverage, not general Flutter or mobile acceptance.
...
This does not enable deferred imports, conditional environments outside the
pinned VM subset, unsupported declarations or experimental enhanced-part features.
