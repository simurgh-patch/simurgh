...
Pass the resulting directories as `--top-accessors-run-dir` and `--top-accessor-gc-run-dir` to `scripts/verify_aot_lab.py`, together with its required base and GC runs. The verifier compares both sides against independently compiled original-source AOT and checks that unchanged classes, storage and callers stay in the baseline; the second fixture observes GC while old AOT retains closures returned by updated accessors. This is host ARM64 experimental coverage, not general Flutter or mobile acceptance.
