...
Compound property updates now collect both resolved read and write elements, including prefixed access. Distinct execution symbols fail closed instead of silently linking only the setter. This is a dependency boundary for future top-level accessor work, not support for replacing explicit top-level getters or setters; those declarations still require stable paired identities, wrappers and native acceptance.
